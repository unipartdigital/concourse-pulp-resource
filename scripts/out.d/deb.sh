...
for deb in $deb_files; do
  ...
  if [[ $file_count -eq "0" ]]; then
    echo "Uploading $deb"
    export TASK_URL=$(http --form POST --ignore-stdin ${ENDPOINT}/pulp/api/v3/content/deb/packages/ file@"$deb" Authorization:"$BASIC_AUTH" | jq -r '.task')
    sleep 2
  ...
  http POST --ignore-stdin ${ENDPOINT}${REPO_HREF}modify/ add_content_units:="[\"${CONTENT_HREF}\"]" Authorization:"$BASIC_AUTH"
  uploaded=1
done
